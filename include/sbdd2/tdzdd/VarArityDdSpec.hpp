/*
 * VarArityDdSpec.hpp - Variable Arity DD Spec for SAPPOROBDD2
 *
 * Provides DD specs with runtime-configurable arity (instead of compile-time).
 * This allows TdZdd-style specs to specify ARITY at construction time.
 */
 ...
 * with variable (runtime) arity.
 *
 * Unlike HybridDdSpec<S, TS, TA, AR> which requires ARITY at compile time,
 * VarArityHybridDdSpec allows setting ARITY at runtime via setArity().
 *
 * Every implementation must have the following functions:
 * - int getRoot(S_State& s, A_State* a)
 * - int getChild(S_State& s, A_State* a, int level, int value)
 *
 * @tparam S the class implementing this class.
 * @tparam TS data type of scalar state.
 * @tparam TA data type of array state elements.
 */
...
protected:
    /**
     * Set the arity (number of child branches per node).
     * Must be called in the constructor of the derived class.
     *
     * @param arity The arity value (1 to 100)
     * @throws DDArgumentException if arity is out of range
     */
    ...
    /**
     * Set the array size for A_State.
     * Must be called in the constructor of the derived class.
     *
     * @param n The array size (non-negative)
     */
...
/**
 * Abstract class of DD specifications using scalar states with variable arity.
 *
 * Every implementation must have the following functions:
 * - int getRoot(T& state)
...
/**
 * Abstract class of DD specifications without states with variable arity.
 *
 * Every implementation must have the following functions:
 * - int getRoot()
