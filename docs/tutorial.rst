...
.. code-block:: cpp
   ...
   int main() {
       ...
       // 隣接する頂点を同時に選ばない制約
       // エッジ (1,2), (2,3), (3,4) を除外
       ZDD exclude_12 = ZDD::single(mgr, 1).product(ZDD::single(mgr, 2));
       ZDD exclude_23 = ZDD::single(mgr, 2).product(ZDD::single(mgr, 3));
       ZDD exclude_34 = ZDD::single(mgr, 3).product(ZDD::single(mgr, 4));
